...
describe('first run and sessions', () => {
  ...
  it('validates game identifiers and isolates mutating state behind CSRF', async () => {
    ...
    const saved = await app.inject({ method: 'PUT', url: '/api/games/2048/session', headers: { cookie: auth.cookie, 'x-csrf-token': auth.csrf }, payload: { state: { board: Array(16).fill(0) }, completed: false } });
  ...
  it('provides deterministic authenticated daily challenges for supported games', async () => {
    ...
    expect(first.json()).toMatchObject({ gameId: 'sudoku' });
