...
- ASVS v5.0.0 V2/V3: Argon2id at 19 MiB, two iterations, one lane; generic failures; IP rate limiting; 30-minute idle and seven-day absolute session expiry; server-side revocation; password-backed recent re-authentication for destructive data, account, and remote-session actions; `HttpOnly`, `SameSite=Strict`, secure `__Host-` cookies outside local development; CSRF proof on every mutation.
- ASVS v5.0.0 V4: every data query is scoped by the authenticated account ID; routes fail closed.
- ASVS v5.0.0 V5: strict per-game Zod state schemas, bounded arrays and values, strict game identifiers, a 64-KiB game-state ceiling, prepared SQLite statements, current-date Daily result validation, and a global body limit.
